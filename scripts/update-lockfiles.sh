...
# interpreter being installed locally. The binding's tests only build the
# extension and parse JSON, so one lockfile covers 3.9 through 3.13.
#
# If uv is not on PATH the script stops and tells you to install it;
# WKSTRATEGYCI_BOOTSTRAP_UV=1 opts into fetching one pinned, checksum-verified
# release into a temporary directory instead. It is opt-in on purpose: piping an
# installer straight into a shell runs whatever is behind that URL at that
# moment, with your privileges, on the machine of everyone who regenerates a
# lockfile.
#
set -euo pipefail
...
if ! command -v uv >/dev/null 2>&1; then
  if [ "${WKSTRATEGYCI_BOOTSTRAP_UV:-0}" != "1" ]; then
    echo "    uv is not on PATH." >&2
    echo "    Install it (https://docs.astral.sh/uv/getting-started/installation/)," >&2
    echo "    or re-run with WKSTRATEGYCI_BOOTSTRAP_UV=1 to fetch uv ${UV_VERSION} here." >&2
    exit 1
  fi
